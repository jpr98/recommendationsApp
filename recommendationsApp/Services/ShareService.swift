...
struct ShareService {
	...
	static func send(to users: [User], stack: [Any], named shareTitle: String) {
		...
		for user in users {
			let ref = Database.database().reference().child("received").child(user.uid).child(currentUser.uid).childByAutoId()
			let dateShared = Date()
			let formatter = DateFormatter()
			formatter.dateFormat = "MMMMd"
			let value: [String: Any] = ["share_title":shareTitle, "date":formatter.string(from: dateShared)]
